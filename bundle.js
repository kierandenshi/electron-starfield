...
/***/ function(module, exports, __webpack_require__) {
	...
	var _starfield = __webpack_require__(2);
	...
	window.onload = function () {
	  var starfield = new _starfield2.default(document.getElementById('canvas'));
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var MAX_DEPTH = 30;
	var NUM_STARS = 800;
	var START_RADIUS = 50; // where to initially position stars relative to centre
	var MOVE_RADIUS = 200; // how far stars move per tick
	var MOVE_SPEED = 0.2; // how fast to decrease depth per tick (best between 0 and 1)
	var RENDER_TICK = 33; // 30fps ~ 33ms
	var STAR_RADIUS = 5;
	...
	var StarField = function () {
	  function StarField(canvas) {
	    ...
	    _.forEach(this.stars, function (val, i, stars) {
	      stars[i] = _this.init_star(true);
	    });
	  ...
	  _createClass(StarField, [{
	    key: 'init_star',
	    value: function init_star() {
	      var starting = arguments.length <= 0 || arguments[0] === undefined ? false : arguments[0];
	    ...
	    value: function render() {
	      ...
	      _.forEach(this.stars, function (val, i, stars) {
	        ...
	        if (stars[i].z <= 0) {
	          stars[i] = _this2.init_star();
	        }

	        // divide move by current z to get parallax effect
	        var mv = MOVE_RADIUS / stars[i].z;

	        // get new pixel coords
	        var px = stars[i].x * mv + halfWidth;
	        var py = stars[i].y * mv + halfHeight;

	        // draw the star if it's still within the canvas bounds
	        if (px >= 0 && px <= _this2.width && py >= 0 && py <= _this2.height) {

	          // make further stars smaller
	          var size = STAR_RADIUS * (1 - stars[i].z / MAX_DEPTH);

	          // make further stars darker
	          var colour = parseInt((1 - stars[i].z / MAX_DEPTH) * 255);
